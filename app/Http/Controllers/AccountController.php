...
class AccountController extends Controller
{
    ...
    public function updateProfileImage(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $imageName = $id . '-' . time() . '.' . $ext; // 1-281024.jpg
            $image->move(public_path('/profile_image/'), $imageName);
            // User::where('id', $id)->update(['image' => $imageName]);
            // session()->flash('success', 'Profile picture is updated successfully');
            ...
            // Crop the best fitting 150x150 ratio and resize to 150x150 pixels
            $image->cover(150, 150);
            $image->toPng()->save(public_path('/profile_image/thumb/' . $imageName));

            // Delete Old Images
            $oldImage = Auth::user()->image;
            if ($oldImage) {
                File::delete(public_path('/profile_image/thumb/') . $oldImage);
                File::delete(public_path('/profile_image/') . $oldImage);
            }
            User::where('id', $id)->update(['image' => $imageName]);
            session()->flash('success', 'Profile Image is Updated');
            return response()->json([
                'status' => true,
                'errors' => []
            ]);
    ...
    public function saveJob(Request $request)
    {
        ...
        if ($validator->passes()) {
            ...
            $jobs->save();
            session()->flash('success', 'Post Job Completed');
            return response()->json([
                'status' => true,
                'errors' => []
            ]);
    ...
    public function updateJob(Request $request, $id)
    {
        ...
        if ($validator->passes()) {
            ...
            session()->flash('success', 'Update Job successfully.');
    ...
    public function profile()
    {
        ...
        // dd($user);
        return view('front.account.account_profile', ['user' => $user]);
    }
